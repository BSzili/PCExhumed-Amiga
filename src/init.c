...
#endif
#ifndef EDUKE32
    int status = loadboard(fileName_1, 0, &initx, &inity, &initz, &inita, &initsect);
    if (status == -2)
        status = loadoldboard(fileName_1, 0, &initx, &inity, &initz, &inita, &initsect);
#else
    vec3_t startPos;
