...
void PlayerInterruptKeys()
{
...
#ifndef EDUKE32
	constexpr int const analogExtent = 1;
#else
    ...
    {
#ifndef EDUKE32
		q16avel = fix16_from_int(info.dyaw);
#else
    ...
    if (aimmode)
#ifndef EDUKE32
        q16horz = fix16_from_int(info.dpitch);
#else
        q16horz = fix16_div(fix16_from_int(info.mousey), F16(64));
#endif
    else
#ifndef EDUKE32
        fvel = -info.dpitch; // TODO this will interfere with joysticks, use CONTROL_MapAnalogAxis
#else
        fvel = -(info.mousey >> 6);
#endif
...
#ifndef EDUKE32
    fix16_t nVPan = nVertPan[nLocalPlayer] + fix16_to_int(q16horz);
...
#ifndef EDUKE32
    nPlayerColor[nPlayer] = *(uint8_t*)(waloff[nTile] + tilesizx[nTile] * tilesizy[nTile] / 2);
#else
...
void RestartPlayer(short nPlayer)
{
	...
	PlayerList[nPlayer].opos.x = sprite[nSprite].x;
	PlayerList[nPlayer].opos.y = sprite[nSprite].y;
...
#ifndef EDUKE32
#ifdef USE_INTERPOLATION
	PlayerList[nPlayer].opos.x = sprite[nPlayerSprite].x;
	PlayerList[nPlayer].opos.y = sprite[nPlayerSprite].y;
