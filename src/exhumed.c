...
void DrawClock()
{
    ...
    while (nVal)
    {
        int v2 = nVal & 0xF;
#ifndef EDUKE32
        int yPos = 32 - tilesizy[v2 + kClockSymbol1] / 2;

        CopyTileToBitmap(v2 + kClockSymbol1, kTile3603, ebp - tilesizx[v2 + kClockSymbol1] / 2, yPos);
#else
...
#ifndef EDUKE32
int myutoa(unsigned int value, char *str)
{
    char tmp[32 + 1];
    char *p1 = tmp, *p2;
    do
    ...
    int len = (intptr_t)p1 - (intptr_t)tmp;
...
{
#ifndef EDUKE32
    static char tempbuf[32 + 1] = {"FPS: "};
    int32_t x = (xdim < 640);

    if (!r_showfps) return;

    //int32_t chars = Bsprintf(tempbuf, "%d fps", lastfps);
    int32_t chars = myutoa(lastfps, tempbuf+5) + 5;
    printext256(windowx2-(chars<<(3-x))/*windowx1+1*/, windowy1+1, whitecol, -1, tempbuf, x);
#else
...
static void GameDisplay(void)
{
	...
	const int smoothRatio = 65536;
#else
    int smoothRatio = calc_smoothratio(totalclock, tclocks);
#endif
#else
...
#ifndef EDUKE32
            else if (Bstrcasecmp(pChar, "cacheprint") == 0) 
            {
...
#ifdef __AMIGA__
	const char *loadText = "Loading, please wait...";
	const size_t loadLength = sizeof(loadText)-1;
	videoClearViewableArea(blackcol);
	printext(10, 10, loadText, kTileFont);
	/*
	int nLoadTile = EXHUMED ? kTile3581 : kTile3352;
	overwritesprite(0, 0, nLoadTile, 0, 2, kPalNormal); 
	suckcache(waloff[nLoadTile]);
	*/
	videoNextPage();
#endif
...
#ifndef EDUKE32
                    lMapZoom += mulscale6(timerOffset, max(lMapZoom, 256));
#else
...
#ifdef __AMIGA__
    // sanity check
    if (nSprite == PlayerList[0].nSprite && (unsigned int)nSector >= kMaxSectors) {
        initprintf("invalid player sector: %d\n", nSector);
        return;
    }
#endif
    DoKenTest();
...
void mydeletesprite(int nSprite)
{
    ...
    if (nSprite == besttarget) {
        besttarget = -1;
#ifdef __AMIGA__
        sPlayerInput[nLocalPlayer].nTarget = -1;
        Ra[nLocalPlayer].nTarget = -1;
#endif
    }
}
...
{
#ifndef EDUKE32
    int nOffs = tilesizy[nDestTile] * xPos;
#else
...
#ifndef EDUKE32
    int destYSize = tilesizy[nDestTile];
    int srcYSize = tilesizy[nSrcTile];
#else
...
#ifndef EDUKE32
    for (int x = 0; x < tilesizx[nSrcTile]; x++)
#else
