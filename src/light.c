...
#ifndef EDUKE32
        palookupfog[kPalRedBrite].r = 248;
        palookupfog[kPalRedBrite].g = 32;
        palookupfog[kPalRedBrite].b = 0;
#else
        palookupfog[kPalRedBrite] = { 248, 32, 0, 0 };
#endif
        hicsetpalettetint(kPalRedBrite, 255, 255, 255, 248, 32, 0, 0);
#ifndef EDUKE32
        palookupfog[kPalGreenBrite].r = 248;
        palookupfog[kPalGreenBrite].g = 32;
        palookupfog[kPalGreenBrite].b = 0;
#else
...
void FadeToWhite()
{
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        for (int j = 0; j < 256; j++)
        {
            ...
            nVal = min(4, 255 - pPal->r);
            if (nVal > 0)
            {
                pPal->r += nVal;
                ebx++;
            }
            nVal = min(4, 255 - pPal->g);
            if (nVal > 0)
            {
                pPal->g += nVal;
                ebx++;
            }
            nVal = min(4, 255 - pPal->b);
            if (nVal > 0)
            {
                pPal->b += nVal;
                ebx++;
