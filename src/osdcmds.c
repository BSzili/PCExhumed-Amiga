...
static int osdcmd_vars(const osdfuncparm_t *parm)
{
    ...
    if (!Bstrcasecmp(parm->name, "showfps")) {
        if (showval) { OSD_Printf("showfps is %d\n", r_showfps); }
        else r_showfps = (atoi(parm->parms[0]) != 0);
        return OSDCMD_OK;
    }
...
static int osdcmd_bind(osdcmdptr_t parm)
{
	...
	{
		for (int i = 0; i < sc_LastScanCode; i++)
		{
	...
	if (parm->numparms==1 && !Bstrcasecmp(parm->parms[0],"listsymbols"))
	{
	...
	if (parm->numparms==0)
	{
		...
		for (int i=0; i<kMaxGameFunctions; i++)
		{
			for (int j=0; j<2; j++)
			{
				if (!(KeyboardKeys[i][j] == 0xff || !KeyboardKeys[i][j]))
				{
					OSD_Printf("%-9s \"%s\"\n", KB_ScanCodeToString(KeyboardKeys[i][j]), gamefunctions[i]);
				}
			}
		}
	...
	kb_scancode key = KB_StringToScanCode(parm->parms[0]);

    // didn't find the key
    if (key == sc_None)
    {
		return OSDCMD_SHOWHELP; // TODO mouse
    }

    if (parm->numparms < 2)
    {
		int func = kMaxGameFunctions;
		for (int i=0; i<kMaxGameFunctions; i++)
		{
			if (KeyboardKeys[i][0] == key || KeyboardKeys[i][1] == key)
			{
				func = i;
				break;
			}
		}
		if (func != kMaxGameFunctions)
			OSD_Printf("%-9s \"%s\"\n", parm->parms[0], gamefunctions[func]);
	...
	int i = CONFIG_FunctionNameToNum(parm->parms[1]);
	if (i != -1)
	{
		KeyboardKeys[i][1] = KeyboardKeys[i][0];
		KeyboardKeys[i][0] = key;
...
static int osdcmd_unbind(osdcmdptr_t parm)
{
	...
	if (key != sc_None)
	{
		...
		for (int i=0; i<kMaxGameFunctions; i++)
		{
			if (KeyboardKeys[i][0] == key || KeyboardKeys[i][1] == key)
			{
				if (KeyboardKeys[i][0] == key)
					KeyboardKeys[i][0] = KeyboardKeys[i][1];
				KeyboardKeys[i][1] = sc_None;
...
static int osdcmd_unbindall(osdcmdptr_t UNUSED(parm))
{
    ...
    for (int i=0; i<kMaxGameFunctions; i++)
        KeyboardKeys[i][0] = KeyboardKeys[i][1] = 0xff;
...
#ifndef EDUKE32
    OSD_RegisterFunction("bind","(bind <key> <function>: associates a keypress with a game function. Type \"bind showkeys\" for a list of keys and \"listsymbols\" for a list of valid game functions.)", osdcmd_bind);
#else
...
#ifndef EDUKE32
    extern void RefreshStatus(void);
    RefreshStatus();
#endif
}
...
void GAME_clearbackground(int numcols, int numrows)
{
    ...
    */
    int xsiz = tilesizx[149];
    int ysiz = tilesizy[149];
    int tx2 = xdim / xsiz;
    int ty2 = nHeight / ysiz;

    for (int x = 0; x <= tx2; x++)
        for (int y = 0; y <= ty2; y++)
            rotatesprite(x*xsiz<<16, y*ysiz<<16, 65536L, 0, 149, 48, 0, 8+16+64, 0, 0, xdim, nHeight);
#else
