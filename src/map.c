...
#ifndef EDUKE32
static inline void GetSpriteExtents(const spritetype* pSprite, int* top, int* bottom)
#else
...
#endif
{
    *top = *bottom = pSprite->z;
    if ((pSprite->cstat & 0x30) != 0x20)
    {
        ...
        int center = height / 2 + ((picanm[pSprite->picnum]>>16)&255);
#else
...
#ifndef EDUKE32
            getzrange(0, 0, 0, pSprite->sectnum, &ceilZ, &ceilHit, &floorZ, &floorHit, (pSprite->clipdist << 2) + 16, CLIPMASK0);
#else
            vec3_t pos;
